...
ReviewWordIdPath = Annotated[int, Path(gt=0)]
...
@router.get(
    "/{user_id}/review/status/{word_id}",
    response_model=ReviewStatusRead,
)
def review_status(
    user_id: UserIdPath,
    word_id: ReviewWordIdPath,
    db: Session = Depends(get_db),
):
    try:
        return {"is_due": get_review_status(db, user_id, word_id)}
    except ValueError as exc:
        raise HTTPException(
...
@router.post("/{user_id}/review/answers", response_model=ReviewAnswerRead)
def review_answer(
    user_id: UserIdPath,
