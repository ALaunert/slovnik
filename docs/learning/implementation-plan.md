...
- P0-05: eight synthetic, explicitly unpublishable example/answer fixtures and a contract decision cover scripts, target spans, variants, context/source references, normalization and holdout leakage. Publish validation now checks the pilot manifest, approved source-backed outcome/role coverage and accepted-variant leakage; the unchanged draft still passes draft checks and fails publish checks. The P1-03b example-table trigger was checked and is not demonstrated by these fixtures.
...
- P1-04a: caller-owned catalog/curriculum publication stages draft content and retirement/activation in one transaction. Preflight resolves referenced owners, written rights, graph/target validity and unrelated IDs; commit rechecks legacy fingerprints and pinned artifact bytes. A stored request fingerprint restricts active retries to matching provenance, and parent locking, final child-set checks and migration guards prevent child insertion/status drift around lexical publication. Fault injection and migration tests protect rollback. The 2026-09-25 full backend run passed 775 tests with disposable PostgreSQL integration enabled; Ruff passed. The synthetic pilot is still inactive.
