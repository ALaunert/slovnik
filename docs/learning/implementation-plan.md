...
- P0-05: eight synthetic, explicitly unpublishable example/answer fixtures and a contract decision cover scripts, target spans, variants, context/source references, normalization and holdout leakage. Validation passed; publish-mode correctly rejects the draft. The P1-03b example-table trigger was checked and is not demonstrated by these fixtures.
- P0-08: multiple-choice construction now normalizes/deduplicates labels, searches beyond duplicate candidates, omits an invalid choice item with fewer than two distinct options, and uses fresh issuance randomness rather than a fixed word-ID seed. Quiz/shadow and frontend quiz regressions passed; semantic distractor review remains editorial.
...
- P1-04a: caller-owned catalog/curriculum publication stages draft content and retirement/activation in one transaction. Preflight resolves all referenced owners, rights for written text/translation, stale legacy fingerprints, graph/target validity and unrelated IDs; repeat activation is idempotent only for matching content. Fault injection and concurrent-child revision tests protect rollback. Full backend verification after integration: 746 passed, 17 skipped; Ruff passed. The disposable PostgreSQL concurrency case remains environment-gated by `SLOVNIK_TEST_POSTGRES_ADMIN_URL`. The synthetic pilot is still inactive.
